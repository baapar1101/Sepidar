IF OBJECT_ID('GNR.fnGetPartyBrokerRemainingPeriodic') IS NOT NULL
	DROP FUNCTION [GNR].[fnGetPartyBrokerRemainingPeriodic]
GO

CREATE FUNCTION [GNR].[fnGetPartyBrokerRemainingPeriodic] (@PeriodType INT, @Calendar INT = 0, @PartyDLRef INT = -1, @ContainsCheque INT = 1, @FiscalYearRef INT, @FromDate DATE = NULL, @ToDate DATE = NULL)
RETURNS TABLE

RETURN

--DECLARE @PeriodType INT = 2, @Calendar INT = 1, @PartyDLRef INT = -1, @ContainsCheque INT = 1, @FiscalYearRef INT = 2, @FromDate DATE = NULL, @ToDate DATE = NULL
